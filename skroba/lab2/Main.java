...
public class Main {
	...
	private static void testMethod(final Class<? extends AbstractGradientIterator> token, final String fileName) {
		try {
			final Constructor<? extends AbstractGradientIterator> constructor = token == ConjugateGradients.class  ?  token.getConstructor(QuadraticFunction.class, int.class) :  token.getConstructor(QuadraticFunction.class);
	...
	private static void testAllMethods(final QuadraticFunction fun, final String ... dir) throws NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
		final String prefix = Arrays.stream(dir).collect(Collectors.joining("/")) + "/";
		testMethod(GradientMethod.class.getConstructor(QuadraticFunction.class), prefix + "GradientMethod", fun);
		testMethod(FastGradientMethod.class.getConstructor(QuadraticFunction.class), prefix + "FastGradientMethod", fun);
		testMethod(ConjugateGradients.class.getConstructor(QuadraticFunction.class, int.class), prefix + "ConjugateGradientMethod", fun, 10);
	}
	
	private static void testMethod(final Constructor<? extends AbstractGradientIterator> constructor, final String fileName, final QuadraticFunction fun, int ... otherArgs) throws InvocationTargetException, InstantiationException, IllegalAccessException {
	...
	private static Pair<String, List<Pair<Vector, Double>>> iterateMethod(final Constructor<? extends AbstractGradientIterator> constructor, final QuadraticFunction fun, int ... otherArgs) throws InvocationTargetException, InstantiationException, IllegalAccessException {
